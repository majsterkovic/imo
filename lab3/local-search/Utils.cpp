...
bool Utils::comparePairs(const std::pair<int, int>& a, const std::pair<int, int>& b) {
    return a.second < b.second;
}

std::vector<std::vector<std::pair<int, int>>> Utils::compute_k_closest_neighbours(const std::vector<std::vector<int>> &dist_mat, int k){
    const int num_cities = dist_mat[0].size();
    std::vector<std::vector<std::pair<int, int>>> closest;

    for (int i = 0; i < num_cities; ++i) {
        std::vector<std::pair<int, int>> tmp_distances;
        std::vector<std::pair<int, int>> distances;
        for (int j = 0; j < num_cities; ++j) {
            if (i != j){
                tmp_distances.emplace_back(j, dist_mat[i][j]);
            }
        }
        std::sort(tmp_distances.begin(), tmp_distances.end(), comparePairs);

        std::vector<std::pair<int, int>> first_k(tmp_distances.begin(), tmp_distances.begin() + k);

        closest.emplace_back(first_k);
    }

    return closest;
}

std::pair<int, int> Utils::find_city(std::vector<int>& cycle1, std::vector<int>& cycle2, int node) {
    int found_cycle = -1;
    int position = -1;
    ...
    return std::make_pair(found_cycle, position);
}